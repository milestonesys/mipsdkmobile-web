...
XPMobileSDK.library.ConnectionResponse = function (xmlString) {
  ...
  if (XMLNodeTextContent(TypeNode) == "Processing") {
    ...
    if (tmp.length > 0) {
      ...
      for (var i = 0, c = paramsNodes.length; i < c; i++) {
        ...
        if (key && value) {
          if (
            this.outputParameters[key] &&
            typeof this.outputParameters[key] == "string"
          ) {
    ...
    if (tmp.length > 0) {
      ...
      if (DiskUsage.length > 0) {
        ...
        var recording = DiskUsage[0].getElementsByTagName(
          "RecordingDiskUsageInBytes",
        );
    ...
    if (XMLNodeTextContent(ResultNode) != "OK") {
      ...
      if (commandNameNode.length > 0) {
        ...
        if (logError) {
          logger.error(
            "Response error " +
              (self.errorString || getError(self.errorCode)) +
              " " +
              (self.errorCode || "") +
              " Complete response: " +
              xmlString,
  ...
  function parseThumbnail(CommandNode) {
    ...
    if (thumbnail) {
      self.thumbnailBase64 =
        "data:image/jpeg;base64," + XMLNodeTextContent(thumbnail);
      return;
    }
  ...
  function parseActions(CommandNode) {
    ...
    self.actions.sort(function (first, second) {
      ...
      if (first.Type == "Outputs" && second.Type == "Events") return -1;
      else if (first.Type == "Events" && second.Type == "Outputs") return 1;

      // if types match, sort by assignment status (assigned outputs go first, then unassigned)
      if (
        first.CameraId != EMPTY_CAMERA_NAME &&
        second.CameraId == EMPTY_CAMERA_NAME
      )
        return -1;
      else if (
        first.CameraId == EMPTY_CAMERA_NAME &&
        ...
        if (subNodes[i].nodeName == "Item") {
          ...
          if (subNodes[i].attributes.length > 0) {
            for (var j = 0; j < subNodes[i].attributes.length; j++) {
              itemData[subNodes[i].attributes[j].name] = safe(
                subNodes[i].attributes[j].value,
              );
            }
          }
          ...
          for (var j = 0; j < subNodes[i].childNodes.length; j++) {
            if (subNodes[i].childNodes[j].nodeName == "Properties") {
              for (
                var k = 0;
                k < subNodes[i].childNodes[j].attributes.length;
                k++
              ) {
                itemData[subNodes[i].childNodes[j].attributes[k].name] = safe(
                  subNodes[i].childNodes[j].attributes[k].value,
                );
              }
  ...
  function attributesToObject(node, options) {
    ...
    for (var i = 0; i < node.attributes.length; i++) {
      ...
      if (
        options &&
        options.numbers &&
        options.numbers.indexOf(attribute.name) != -1
      ) {
        result[attribute.name] = Number(attribute.value);
      } else if (
        options &&
        options.dates &&
        options.dates.indexOf(attribute.name) != -1
      ) {
