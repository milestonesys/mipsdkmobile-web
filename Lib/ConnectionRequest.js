(function (undefined) {
  ...
  var connectionRequest = function (
    ...
    var connectionURL =
      parseURL(XPMobileSDK.library.Connection.server) +
      XPMobileSDKSettings.communicationChanel;
    ...
    function onComplete(response) {
      if (
        ...
        (command == "LiveMessage" &&
          Date.now() - lastLiveMessageRequestTimestamp <
            XPMobileSDKSettings.liveMessageMinimumInterval)
      ) {
        XPMobileSDK.removeObserver(self);
        return;
      }
