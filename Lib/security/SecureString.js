...
XPMobileSDK.library.SecureString = {
  ...
  encrypt: function (string) {
    ...
    var params = { iv: iv };
    if (
      Settings.DefaultEncryptionPadding &&
      CryptoJS.pad[Settings.DefaultEncryptionPadding]
    ) {
    ...
    return (
      cip.iv.toString(CryptoJS.enc.Base64) +
      ":" +
      cip.ciphertext.toString(CryptoJS.enc.Base64)
    );
  },
  ...
  generateKey: function () {
    ...
    if (!this.sharedKey) {
      if (XPMobileSDK.library.Connection.dh) {
        sharedKey = this.sharedKey = XPMobileSDK.library.Connection.dh
          .getSharedKey()
          .toUpperCase();
      } else if (XPMobileSDK.library.CHAP.sharedKey) {
        sharedKey = this.sharedKey =
          XPMobileSDK.library.CHAP.sharedKey.toUpperCase();
      }
    } else {
      sharedKey = this.sharedKey;
    }
