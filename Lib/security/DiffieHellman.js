...
XPMobileSDK.library.DiffieHellman = function (params) {
   ...
   * @access		private
   */
  var primeKeyBigInt = str2bigInt(
    primeKey[XPMobileSDKSettings.primeLength],
    16,
    1,
  );
  ...
  this.createPublicKey = function () {
    ...
    var byteArrayKey = str2byteArray(
      bigInt2str(powMod(generator, randKey, primeKeyBigInt), 16),
    );
    byteArrayKey.push(0);
  ...
  this.setServerPublicKey = function (publicKey) {
    ...
    serverKey = CryptoJS.enc.Base64.parse(
      Base64.encodeArray(reversedServerKey),
    ).toString();
  };
  ...
  this.getSharedKey = function () {
    var secretKey = str2byteArray(
      bigInt2str(
        powMod(str2bigInt(serverKey, 16, 1), randKey, primeKeyBigInt),
        16,
      ),
    );
  ...
  this.encodeString = function (str) {
    ...
    return CryptoJS.AES.encrypt(str, key, params).ciphertext.toString(
      CryptoJS.enc.Base64,
    );
  };
};
