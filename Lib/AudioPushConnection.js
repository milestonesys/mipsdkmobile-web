XPMobileSDK.library.AudioPushConnection = function (
  itemIds,
  sampleRate,
  successCallback,
  errorCallback,
  ...
  var callbacks = {
    ...
    onAjaxFailure: function () {
      logger.error("ERROR in ajax request for audio push");
    },
  };

  var stream, streamRequest;
  ...
  function open(successCallback, errorCallback, onAjaxFailure) {
    ...
    streamRequest = XPMobileSDK.requestAudioStreamIn(
      itemIds,
      options,
      streamRequestCallback,
      streamRequestErrorCallback,
    );
  }
  ...
  function send(pcmData) {
    ...
    bufferView.set(
      XPMobileSDK.library.Bytes.fromInt(new Date().getTime(), 8),
      16,
    ); // Timestamp
