...
class AudioStream extends BaseElement {
  ...
  constructor() {
    ...
    this.addEventListener(
      "switchToPlayback",
      this.onSwitchToPlayback.bind(this),
    );
    this.addEventListener("switchToLive", this.onSwitchToLive.bind(this));
    this.addEventListener(
      "setAudioStreamOption",
      this.onSetAudioStreamOption.bind(this),
    );
    this.addEventListener(
      "setContinueToPlay",
      this.setContinueToPlay.bind(this),
    );
  ...
  startAudioStream(event) {
    ...
    if (this._optionSlot) {
      this._optionSlot.addEventListener(
        "slotchange",
    ...
    this.params.onStart = () => {
      this.setState(this.stateId, {
        id: this.params.cameraId,
        state: "started",
        live: this.audioController.isLive,
        source: this.audioController.audioSource,
      });
    };
    this.params.onStop = () => {
    ...
    this.params.onPlayingCallback = () => {
      this.dispatchEvent(
        new CustomEvent("onAudioPlaying", {
          bubbles: true,
          composed: true,
    ...
    this.params.onStalledCallback = () => {
      this.dispatchEvent(
        new CustomEvent("onAudioStalled", {
          bubbles: true,
          composed: true,
          detail: { component: name },
        }),
    ...
    this.dispatchEvent(
      new CustomEvent("audioInitialized", {
        detail: { selectAudioSource: this.audioSourceDropDown },
      }),
    );
  ...
  onErrorCallback(error) {
    this.dispatchEvent(
      new CustomEvent("audioError", { detail: { error: error } }),
    );
  ...
  setContinueToPlay() {
    if (this.stateId) {
      this.getState(this.stateId, (data) => {
        this.state = data;
        if (
          data.id == this.params.cameraId &&
          data.state == "started" &&
          data.live == this.audioController.isLive
        ) {
          setTimeout(() => {
            this.show &&
              !this.audioController.isDisabled &&
  ...
  onSetAudioStreamOption(event) {
    if (this.audioController) {
      this.audioController.setAudioStreamOption(
        event.detail.name,
        event.detail.value,
      );
    } else {
  ...
  onSwitchToPlayback(event) {
    setTimeout(() => {
      this.audioController &&
        this.audioController.switchToPlayback(
          event.detail.playbackControllerId,
          this.getAudioSourceState(),
        );
    }, 100); // Give some time for the shadow dom elements to be ready
  }
  ...
  destroy() {
    ...
    this.removeEventListener(
      "mouseleave",
      this.hideAudioSelectLabel.bind(this),
    );
    ...
    this.removeEventListener(
      "deleteAudioState",
      this.deleteAudioState.bind(this),
    );
    this.playbackControllerId = null;
  }
}
