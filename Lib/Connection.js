...
Connection = function () {
  ...
  this.initialize = function (storage) {
    if (storage) {
      ...
      self.directStreamingClient = self.storage.getItem(
        "directStreamingClient",
      );
      self.directStreamingServer = self.storage.getItem(
        "directStreamingServer",
      );
  ...
  var connectCallback = function (connectionRequest) {
    ...
    if (!connectionResponse || connectionResponse.isError) {
      var failedResponseParameter =
        connectionResponse && connectionResponse.error;
      callMethodOnObservers(
        "connectionFailedToConnect",
        failedResponseParameter,
      );
      connectionRequest.options.failCallback &&
        connectionRequest.options.failCallback(failedResponseParameter);
    } else {
      self.connectionId = connectionResponse.outputParameters.ConnectionId;
      self.serverTimeout = parseInt(
        connectionResponse.outputParameters.Timeout,
      );
      self.serverId = connectionResponse.outputParameters.ServerId;

      if (self.storage) {
        self.webSocketServer =
          connectionResponse.outputParameters.WebSocketSupport === "Yes";
        self.storage.setItem("webSocketServer", self.webSocketServer);

        if (
          typeof self.storage.getItem("webSocketBrowser") === "boolean" &&
          !!window.WebSocket
        ) {
          self.webSocketBrowser = self.storage.getItem("webSocketBrowser");
        } else {
        ...
        if (
          typeof self.storage.getItem("directStreamingClient") === "boolean"
        ) {
          self.directStreamingClient = self.storage.getItem(
            "directStreamingClient",
          );
      ...
      if (connectionResponse.outputParameters.SecurityEnabled) {
        self.SecurityEnabled =
          connectionResponse.outputParameters.SecurityEnabled;
      }

      if (connectionResponse.outputParameters.PublicKey) {
        self.PublicKey = connectionResponse.outputParameters.PublicKey;
        self.dh &&
          self.dh.setServerPublicKey(
            connectionResponse.outputParameters.PublicKey,
          );
      }

      if (
        connectionResponse.outputParameters.CHAPSupported &&
        connectionResponse.outputParameters.CHAPSupported === "Yes"
      ) {
      ...
      if (connectionResponse.outputParameters.ServerProductCode) {
        self.ServerProductCode =
          connectionResponse.outputParameters.ServerProductCode;
      }
      ...
      connectionRequest.options.successCallback &&
        connectionRequest.options.successCallback(successResponseParameter);
    }
  };
  ...
  this.Login = function (params, successCallback, failCallback) {
    ...
    logger.log(
      "Log in with authentication type " +
        params.LoginType +
        " username " +
        params.Username +
        " password " +
  ...
  var loginCallback = function (connectionRequest) {
    ...
    if (!connectionResponse || connectionResponse.isError) {
      if (
        connectionResponse &&
        connectionResponse.error.code ===
          XPMobileSDK.library.ConnectionError.SecondStepAuthenticationRequired
      ) {
        var ssarParameter =
          connectionResponse.outputParameters.SecondStepAuthenticationProvider;
        callMethodOnObservers("connectionRequiresCode", ssarParameter);
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(ssarParameter);
      } else {
  ...
  var proceedWithLogin = function (connectionResponse, successCallback) {
    self.directStreamingServer =
      connectionResponse.outputParameters.DirectStreamingLive === "Yes";
    self.storage.setItem("directStreamingServer", self.directStreamingServer);

    self.exportToAvi =
      connectionResponse.outputParameters.ExportToAvi === "Yes";
    ...
    self.exportToMkv =
      connectionResponse.outputParameters.ExportToMkv === "Yes";
    ...
    callMethodOnObservers(
      "connectionDidLogIn",
      connectionResponse.outputParameters,
    );
    successCallback && successCallback();
  };
  ...
  this.requestCode = function (successCallback, failCallback) {
    var params = {};
    return self.sendCommand(
      "RequestSecondStepAuthenticationPin",
      params,
      { successCallback: successCallback },
      requestCodeCallback,
      failCallback,
  ...
  var requestCodeCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error requesting validation code.",
      connectionRequest.options.successCallback,
    );
  };
  ...
  this.verifyCode = function (code) {
    ...
    return self.sendCommand(
      "VerifySecondStepAuthenticationPin",
      params,
      null,
      verifyCodeCallback,
    );
  ...
  var verifyCodeCallback = function (connectionRequest) {
    ...
    if (!connectionResponse || connectionResponse.isError) {
      if (
        connectionResponse &&
        connectionResponse.error.code ===
          XPMobileSDK.library.ConnectionError.SecondStepAuthenticationCodeError
      ) {
  ...
  this.Disconnect = function (params, successCallback, failCallback) {
    ...
    params = params || {};
    var connectionRequest = self.sendCommand(
      "Disconnect",
      params,
      { successCallback: successCallback },
      logOutCallback,
  ...
  this.getViews = function (viewId, successCallback, failCallback) {
    return self.sendCommand(
      "GetViews",
      { ViewId: viewId },
      { successCallback: successCallback, ViewId: viewId },
      getViewsCallback,
      failCallback,
    );
  };
  ...
  var getViewsCallback = function (connectionRequest) {
    callbackAfterRequest(connectionRequest, null, function () {
      var subViews = [];
      var subViewsNodes =
        connectionRequest.response.subItems.getElementsByTagName("Item");
      ...
      connectionRequest.options.successCallback &&
        connectionRequest.options.successCallback(view);
    });
  };
  ...
  var getAllViewsCallback = function (connectionRequest) {
    callbackAfterRequest(
      ...
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.items,
          );
      },
    );
  };
  ...
  this.getAllCameras = function (successCallback, failCallback) {
    return self.sendCommand(
      ...
      {
        ItemKind: "Camera",
        Hierarchy: "UserDefined",
        IncludeRelatedDevices: "Yes",
      },
      { successCallback: successCallback },
      getAllCamerasCallback,
      failCallback,
    );
  };
  ...
  var getAllCamerasCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error executing GetItems on the server.",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.items,
  ...
  var requestPushStreamCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error requesting video push stream from the server.",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.outputParameters,
          );
      },
    );
  };
  ...
  this.requestStream = function (
    ...
    var params = {
      ...
      SignalType:
        options.signal === XPMobileSDK.interfaces.VideoConnectionSignal.playback
          ? "Playback"
          : "Live",
      MethodType: "Push",
      Fps: maxFps, // This doesn't work for Pull mode, but we have to supply it anyway to keep the server happy
      ComprLevel: options.jpegCompressionLevel
        ? options.jpegCompressionLevel
        : 70,
    ...
    if (
      XPMobileSDK.features.MultiCameraPlayback &&
      options.playbackControllerId
    ) {
  ...
  var requestStreamCallback = function (connectionRequest) {
    let messageString =
      "Error starting stream for camera " + connectionRequest.options.cameraId;
    if (
      connectionRequest.response.errorCode ===
    ...
    callbackAfterRequest(connectionRequest, messageString, function () {
      ...
      logger.log(
        "Server prepared video ID " +
          videoId +
          " for camera " +
          connectionRequest.options.cameraId,
      );

      let videoConnection = new VideoStream(videoId, connectionRequest);
      if (connectionRequest.options.reuseConnection) {
        XPMobileSDK.library.VideoConnectionPool.addCamera(
          connectionRequest.options.cameraId,
          videoConnection,
        );
      }
      connectionRequest.options.successCallback &&
        connectionRequest.options.successCallback(videoConnection);
    });
  };
  ...
  this.RequestAudioStreamIn = function (params, successCallback, failCallback) {
    return self.sendCommand(
      "RequestAudioStreamIn",
      params,
      { successCallback: successCallback },
      requestAudioStreamInCallback,
      failCallback,
    );
  };
   ...
   * @return {ConnectionRequest} - the ConnectionRequest object
   */
  this.requestAudioStreamIn = function (
    itemId,
    options,
    successCallback,
    failCallback,
  ...
  var closeVideoConnection = function (videoConnection) {
    ...
    videoConnection.isReusable &&
      XPMobileSDK.library.VideoConnectionPool.removeCamera(
        videoConnection.cameraId,
        videoConnection.videoId,
      );
  };
  ...
  var restartVideoConnection = function (videoConnection) {
    ...
    self.sendCommand(
      "RequestStream",
      videoConnection.request.parameters,
      videoConnection.request.options,
      requestStreamCallback,
    );
  };
   ...
   * @return the request object
   */
  this.requestAudioStream = function (
    microphoneId,
    options,
    successCallback,
    failCallback,
  ...
  var requestAudioStreamCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error starting stream for microphone " +
        connectionRequest.options.microphoneId,
      function () {
        var streamId = connectionRequest.response.outputParameters.StreamId;
        logger.log(
          "Server prepared stream ID " +
            streamId +
            " for microphone " +
            connectionRequest.options.microphoneId,
        );
        ...
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(connectionRequest);
      },
    );
  };
  ...
  this.toggleDirectStreaming = function (enabled) {
    ...
    XPMobileSDK.library.VideoConnection.instances.forEach(
  ...
  var fps = new (function () {
    ...
    var change = function (fps, successCallback, failCallback) {
      for (
        var i = 0, camera;
        i < XPMobileSDK.library.VideoConnectionPool.cameras.length;
        i++
      ) {
        camera = XPMobileSDK.library.VideoConnectionPool.cameras[i];

        if (!camera.videoConnection || !camera.videoConnection.videoId)
          continue;
    ...
    callbackAfterRequest(
      ...
      function () {
        if (connectionRequest.response.thumbnailBase64) {
          connectionRequest.options.successCallback &&
            connectionRequest.options.successCallback(
              connectionRequest.response.thumbnailBase64,
            );
        }
      },
    );
  ...
  var getThumbnailByTimeCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting thumbnail by time",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.outputParameters.Thumbnail,
  ...
  var getDBStartTimeCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting recording time",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.outputParameters.Time,
          );
      },
    );
  };
  ...
  var getNextSequenceCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting sequences",
      function () {
        if (connectionRequest.response.sequences.length > 0) {
          connectionRequest.options.successCallback &&
            connectionRequest.options.successCallback(
              connectionRequest.response.sequences[0],
            );
        } else {
          connectionRequest.options.successCallback &&
            connectionRequest.options.successCallback(null);
        }
      },
    );
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.getSequencesInInterval = function (
    itemId,
    startTime,
    endTime,
    investigationId,
    ...
    return self.sendCommand(
      ...
      params,
      { successCallback: successCallback },
      getSequencesInIntervalCallback,
      failCallback,
    );
  };
  ...
  var getSequencesInIntervalCallback = function (connectionRequest) {
    callbackAfterRequest(
      ...
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.sequences,
          );
      },
    );
  };
   ...
   */
  this.getSequencesForView = function (
    itemId,
    startTime,
    endTime,
  ...
  var getUserExportsCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting user exports",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.exports,
          );
      },
    );
  };
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.createExportDownloadLink = function (
    exportId,
    investigationId,
    exportType,
    successCallback,
    ...
    return self.sendCommand(
      ...
      params,
      { successCallback: successCallback },
      createExportDownloadLinkCallback,
      failCallback,
    );
  };
  ...
  var createExportDownloadLinkCallback = function (connectionRequest) {
    callbackAfterRequest(
      ...
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.outputParameters.ExportLink,
          );
      },
    );
  };
  ...
  var getExportCallback = function (connectionRequest) {
    callbackAfterRequest(
      ...
      function () {
        ...
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.exports[0],
          );
      },
    );
  };
  ...
  var getOutputsAndEventsCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting outputs and events",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.actions,
          );
      },
    );
  };
  ...
  var getServerStatusCallback = function (connectionRequest) {
    callbackAfterRequest(
      ...
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.ServerStatus,
          );
      },
    );
  };
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.triggerOutputOrEvent = function (
    objectId,
    triggerType,
    successCallback,
    failCallback,
  ...
  var triggerOutputOrEventCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error triggering output or event.",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback();
      },
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.getCameraCapabilities = function (
    cameraId,
    successCallback,
    failCallback,
  ) {
  ...
  var getCameraCapabilitiesCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting camera capabilities",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.outputParameters,
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.createPlaybackController = function (
    params,
    successCallback,
    failCallback,
  ) {
  ...
  var createPlaybackControllerCallback = function (connectionRequest) {
    ...
    callbackAfterRequest(
      connectionRequest,
      "Error creating playback controller",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(videoConnection);
      },
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.changeMultipleStreams = function (
    params,
    successCallback,
    failCallback,
  ) {
    return self.sendCommand(
      "ChangeMultipleStreams",
      params,
      { successCallback: successCallback },
      changeMultipleStreamsCallback,
      failCallback,
    );
  };
  ...
  var getInvestigationCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting investigation",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.items[0],
          );
      },
    );
  };
  ...
  var createInvestigationCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error creating investigation to the server.",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.outputParameters,
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.updateInvestigationData = function (
    params,
    successCallback,
    failCallback,
  ) {
  ...
  var updateInvestigationCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error updating investigation to the server.",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.outputParameters,
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.deleteInvestigation = function (
    investigationId,
    successCallback,
    failCallback,
  ) {
  ...
  var deleteInvestigationCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error deleteing investigation from the server.",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            !connectionRequest.response.isError,
          );
      },
    );
  };
  ...
  this.cancelInvestigation = function (investigationId) {
    return self.sendCommand(
      "CancelInvestigationUpdate",
      { ItemId: investigationId },
      null,
      cancelInvestigationCallback,
    );
  };
  ...
  var cancelInvestigationCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error canceling investigation update",
    );
  };
   ...
   * @param {Function} failCallback: function that is called when the command execution has failed and the error is passed as a parameter.
   */
  this.startInvestigationExport = function (
    investigationId,
    exportType,
    includeAudio,
    password,
    ...
    return self.sendCommand(
      ...
      params,
      { successCallback: successCallback },
      startInvestigationExportCallback,
      failCallback,
    );
  };
  ...
  var startInvestigationExportCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error starting investigation export.",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.outputParameters.ExportId,
          );
      },
    );
  };

  this.deleteInvestigationExport = function (
    investigationId,
    exportType,
    successCallback,
    failCallback,
    ...
    return self.sendCommand(
      "DeleteInvestigationExport",
      params,
      { successCallback: successCallback },
      deleteInvestigationExportCallback,
      failCallback,
    );
  };
  ...
  var deleteInvestigationExportCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error deleting investigation export.",
      connectionRequest.options.successCallback,
    );
  };
  ...
  var getAlarmDataSettingsCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting alarm data settings",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.items,
  ...
  var getAlarmUsersCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting permitted users for alarm",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.items,
  ...
  this.prevCarouselCamera = function (videoId) {
    return self.sendCommand(
      "ControlCarousel",
      { VideoId: videoId, CarouselCommand: "PreviousCamera" },
      null,
      prevCarouselCameraCallback,
    );
  };
  ...
  var prevCarouselCameraCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting prev camera from carousel",
    );
  };
  ...
  this.nextCarouselCamera = function (videoId) {
    return self.sendCommand(
      "ControlCarousel",
      { VideoId: videoId, CarouselCommand: "NextCamera" },
      null,
      nextCarouselCameraCallback,
    );
  };
  ...
  var nextCarouselCameraCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error getting next camera from carousel",
    );
  };
  ...
  this.pauseCarousel = function (videoId) {
    return self.sendCommand(
      "ControlCarousel",
      { VideoId: videoId, CarouselCommand: "PauseCarousel" },
      null,
      pauseCarouselCallback,
    );
  };
  ...
  this.resumeCarousel = function (videoId) {
    return self.sendCommand(
      "ControlCarousel",
      { VideoId: videoId, CarouselCommand: "ResumeCarousel" },
      null,
      resumeCarouselCallback,
    );
  };
  ...
  this.RegisterForNotifications = function (
    ...
    return self.sendCommand(
      ...
      params,
      { successCallback: successCallback },
      registerForNotificationsCallback,
      failCallback,
    );
  };
  ...
  var registerForNotificationsCallback = function (connectionRequest) {
    callbackAfterRequest(
      connectionRequest,
      "Error register for notifications.",
      function () {
        connectionRequest.options.successCallback &&
          connectionRequest.options.successCallback(
            connectionRequest.response.items,
  ...
  this.RequestBookmarkCreation = function (
    params,
    successCallback,
    failCallback,
  ) {
    return self.sendCommand(
      "RequestBookmarkCreation",
      params,
      { successCallback: successCallback },
      requestBookmarkCreationCallback,
      failCallback,
    );
  };
   ...
   * @param callback
   */
  var callbackAfterRequest = function (
    connectionRequest,
    errorMessage,
    callback,
  ) {
    ...
    if (!connectionResponse || connectionResponse.isError) {
      if (connectionRequestResponseIsTerminal(connectionRequest)) {
        logger.error(
          "The application has lost connection due to connectionRequestResponseIsTerminal",
        );
  ...
  this.sendCommand = function (
    ...
    if (
      ...
      self.SecurityEnabled === "Yes" &&
      self.CHAPSupported === "Yes"
    ) {
      ...
      if (challenge.Challenge && challenge.ChalAnswer) {
        ...
        XPMobileSDK.library.Connection.state ===
        XPMobileSDK.library.ConnectionStates.working
      ) {
    ...
    logger.log(
      "Sending " + commandName + " on " + new Date() + "with ",
      requestParams,
    );

    var connectionRequest = new XPMobileSDK.library.ConnectionRequest(
      commandName,
      getNextSequenceID(),
      requestParams,
      options,
  ...
  var callMethodOnObservers = function () {
    ...
    observers
      .filter((observer) => observer[methodName])
  ...
  var scheduleLiveMessage = function () {
    ...
    self.liveMessageTimer = setInterval(
      self.sendLiveMessage,
      (self.serverTimeout * 1000) / 3,
    );
  };
  ...
  var liveMessageFailCallback = function () {
    ...
    scheduledDisconnectOnFail = setTimeout(
      disconnectOnFail,
      self.serverTimeout * 1000,
    );
  };
  ...
  var liveMessageCallback = function (connectionRequest) {
    ...
    if (
      connectionResponse.outputParameters.FolderDefinitionsChanged === "Yes" ||
    ...
    if (
      connectionResponse.outputParameters.CameraDefinitionsChanged === "Yes"
    ) {
    ...
    for (i in features) {
      switch (i) {
        ...
        case "ServerDescription":
          ...
          break;
        default:
          var value = isNaN(Number(features[i]))
            ? features[i] === "Yes"
            : Number(features[i]);
          data[i] = value;
          break;
  ...
  var connectionRequestResponseIsTerminal = function (connectionRequest) {
    ...
    if (
      ...
      connectionResponse.errorCode ===
        XPMobileSDK.library.ConnectionError.WrongID ||
      connectionResponse.errorCode ===
        XPMobileSDK.library.ConnectionError.ChallengesLimitReached ||
