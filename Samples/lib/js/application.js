...
var Application = new (function () {
  ...
  function Camera(event) {
    ...
    if (event.currentTarget.classList.contains("pauseButton")) {
      var Id = event.currentTarget.parentNode.parentNode.id.replace(
        "container",
        "",
      );
    } else {
    ...
    document
      ...
      .removeEventListener("click", Camera);

    if (attachContainerClick) {
      document
        .getElementById("container" + Id)
        .addEventListener("click", switchToPlayback);
    }

    document
      .querySelector("#container" + Id + " .bottombar")
      .setAttribute("style", "display:none;");
    document
      .querySelector("#container" + Id + " .bottombar")
      .classList.remove("onPause");
    document
      .querySelector("#container" + Id + " .playBackButton")
      .addEventListener("click", playBackwardTrigger);
    document
      .querySelector("#container" + Id + " .playForwardButton")
      .addEventListener("click", playForwardTrigger);
    ...
    function videoConnectionReceivedFrame(frame) {
      if (!drawing && frame.dataSize > 0) {
        ...
        if (
          !isLive &&
          frame.timestamp.getTime() != playbackTimestamp.getTime()
        ) {
    ...
    function switchToPlayback() {
      ...
      document
        .getElementById("container" + Id)
        .removeEventListener("click", switchToPlayback);
      ...
      streamRequest = XPMobileSDK.RequestStream(
        RequestStreamParams(Id, "Playback"),
        requestStreamCallback,
        null,
      );
    }
    ...
    function switchToLive(e) {
      ...
      document
        .getElementById("container" + Id)
        .removeEventListener("click", switchToLive);
      document
        .getElementById("container" + Id)
        .addEventListener("click", switchToPlayback);
      document
        .getElementById("container" + Id)
        .classList.add("playing", "live");
      document
        .querySelector("#container" + Id + " .bottombar")
        .setAttribute("style", "display:none;");
      document
    ...
    function showPlaybackControls() {
      ...
      document
        .querySelector("#container" + Id + " .bottombar")
        .classList.add("onPause");

      document
        .querySelector("#container" + Id + " .pauseButton")
        .setAttribute("title", "Live");
      document
        .querySelector("#container" + Id + " .pauseButton")
        .addEventListener("click", switchToLive);
    }
    ...
    function updatePlaybackButtons() {
      var playForwardButton = document.querySelector(
        "#container" + Id + " .playForwardButton",
      );
      var playBackButton = document.querySelector(
        "#container" + Id + " .playBackButton",
      );
    ...
    function updateTime(timestamp) {
      ...
      document.querySelector("#container" + Id + " .playTimeIndex").innerHTML =
        formattedTime;
    }
  }
})();
