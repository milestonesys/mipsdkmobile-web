(function () {
  var thirdPartyLoginManager = function (settings) {
    ...
    function initInternal() {
      ...
      self.defaultServer.textContent =
        "(defaults to: " +
    ...
    function connectionDidConnect() {
      ...
      if (self.ExternalLoginProgress) {
        ...
        const code_verifier = XPMobileSDK.sessionStorage.getItem(
        ...
        const scope = encodeURIComponent(
          thirdPartyLoginManager.ExternalLoginConfig.scope,
        );
    ...
    function initExternalLogin() {
      let serverName =
        XPMobileSDKSettings.MobileServerURL ||
        thirdPartyLoginManager.ServerName;
      serverName = !serverName.endsWith("/") && serverName.concat("/");

      XPMobileSDK.library.Ajax.Request(
        serverName +
          thirdPartyLoginManager.IdpRelativePath +
          "" +
          thirdPartyLoginManager.IdpConfiguration,
        {
          method: "get",
    ...
    function onGetExternalProviders(response) {
      try {
        ...
        const response_type =
          thirdPartyLoginManager.ExternalLoginConfig.responseType;
        const code_challenge_method =
          thirdPartyLoginManager.ExternalLoginConfig.codeChallengeMethod;
        const client_id = thirdPartyLoginManager.ExternalLoginConfig.clientId;
        ...
        providers.forEach((provider) => {
          const acrValue = encodeURIComponent(
            `${thirdPartyLoginManager.ExternalLoginConfig.acrValue}:${provider.loginProviderId}`,
          );
          ...
          element.setAttribute(
            "href",
            self.authorization_endpoint +
              `?response_type=${response_type}&nonce=${codeVerified}&state=${state}&code_challenge=${codeChallenge}&code_challenge_method=${code_challenge_method}&client_id=${client_id}&scope=${scope}&redirect_uri=${redirectURL}&acr_values=${acrValue}&culture=${culture}&prompt=${prompt}`,
          );
          element.innerHTML = `Log in with ${provider.displayName}`;
          element.addEventListener("click", function () {
            XPMobileSDK.sessionStorage.setItem(
              "externalCodeVerifier",
              codeVerified,
            );
            XPMobileSDK.sessionStorage.setItem("externalState", state);
          });
          element.parentElement.classList.contains("hide") &&
    ...
    function onGetToken(response) {
      try {
        const { access_token, id_token, refresh_token } = JSON.parse(
          response.responseText,
        );
        XPMobileSDK.externalLogin(
          thirdPartyLoginManager.ExternalLoginConfig.clientId,
          id_token,
          access_token,
          refresh_token,
          thirdPartyLoginManager.ExternalLoginConfig.loginType,
          {},
        );
      } catch (e) {
    ...
    function init() {
      ...
      self.connectionFailedToLogIn =
        settings.connectionFailedToLogIn || function () {};
      ...
      if (code && state) {
        ...
        self.token_endpoint =
          XPMobileSDK.sessionStorage.getItem("tokenEndPoint");
        self.code = code;
