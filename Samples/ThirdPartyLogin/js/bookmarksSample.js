...
var Application = new (function () {
  ...
  function createBookmark(event) {
    ...
    XPMobileSDK.CreateBookmark(
      objRequest.requestData,
      objRequest.successCreateBookmark,
      objRequest.failCreateBookmark,
    );
  }
  ...
  let successCreateBookmark = function (event) {
    setTimeout(() => {
      allBookmarksTable.dispatchEvent(
        new CustomEvent("getBookmarks", { detail: { data: "" } }),
      );
    }, 100);

    document.dispatchEvent(
      new CustomEvent("createBookmark", { detail: { success: true } }),
    );
  };

  let failCreateBookmark = function (event) {
    document.dispatchEvent(
      new CustomEvent("createBookmark", { detail: { success: false } }),
    );
  };
  ...
  function getAllBookmarks(event) {
    ...
    getAllBookmarks = function (callback, errorCallback) {
      XPMobileSDK.getBookmarks(
        { Count: 30 },
        parseGetBookmarksResult,
        failGetAllBookmarks,
      );
    };
